...
# write a script that enumerates all the possible partitions of a dimension vector
# a partition is a unique way of writing a dimension vector as a sum of subdimension vectors

function enumerate_partitions(d::AbstractVector{Int})
  # we will use a recursive approach to enumerate all partitions of d
  if all(di == 0 for di in d)
    return [[]] # the only partition of the zero vector is the empty partition
  end

  partitions = []
  for e in QuiverTools.all_subdimension_vectors(d; nonzero=true)
    subpartitions = enumerate_partitions(d - e)

    for sp in subpartitions
      push!(partitions, sort!([e, sp...]))
    end
  end
