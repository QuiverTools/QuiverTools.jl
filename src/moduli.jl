...
function show(io::IO, M::QuiverModuli)
    print(io, "Moduli space of $(M.condition) representations of $(M.Q) with dimension vector $(M.d) and stability parameter $(M.theta)")
end
...
function dimension(M::QuiverModuli)
    ...
    elseif M.condition == "semistable"
        if has_semistables(M.Q, M.d, M.theta)
            return maximum(dimension_of_luna_stratum(tau) for tau in all_luna_types(M.Q, M.d, M.theta))
        end
    else
        return "-∞" # how?
    end
...
"""
Checks if the moduli space is infinitesimally rigid by verifying wether the Teleman quantization criterion of [arXiv:2311.17003](https://doi.org/10.48550/arXiv.2311.17003).

Only a sufficent criterion is implemented, so the function may return "not known" even if the moduli space is rigid.
"""
